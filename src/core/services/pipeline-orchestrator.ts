 ...
 *
 * This is the heart of the autonomaton. Every request flows through here:
 *
...
type Dispatch = (action: BaseAppAction) => void

/**
 * Response generator interface for recipe injection
 * Recipes provide their own simulated responses for demo mode
 */
export interface ResponseGenerator {
  getSimulatedResponse: (intent: string, input: string) => string
  generatePatternDescription?: (intent: string) => string
}

// Default response generator (generic)
const defaultResponseGenerator: ResponseGenerator = {
  getSimulatedResponse: (intent: string, input: string) => {
    return `Processed "${input}" via intent "${intent}".

This is a generic response. Configure a recipe for domain-specific responses.`
  },
  generatePatternDescription: generatePatternDescription,
}

// Registry for recipe response generators
let activeResponseGenerator: ResponseGenerator = defaultResponseGenerator

/**
 * Register a recipe's response generator
 */
export function registerResponseGenerator(generator: ResponseGenerator): void {
  activeResponseGenerator = generator
}

/**
 * Reset to default response generator
 */
export function resetResponseGenerator(): void {
  activeResponseGenerator = defaultResponseGenerator
}
...
export async function processInteraction(
  ...
  // Early Andon Cord — Validate API key at compilation, not execution
  ...
  if (willIncrementPattern) {
    ...
    if (flywheelEligible) {
      ...
      if (shouldProposeSkill(decision.intent, { ...state.patternCounts, [decision.intent]: newCount }, state.skills, state.routingConfig)) {
        const patternDesc = activeResponseGenerator.generatePatternDescription
          ? activeResponseGenerator.generatePatternDescription(decision.intent)
          : generatePatternDescription(decision.intent)
        dispatch({
          type: 'PROPOSE_SKILL',
          intent: decision.intent,
          pattern: patternDesc,
...
export async function completeExecution(
  ...
  if (state.mode === 'demo') {
    ...
    response = activeResponseGenerator.getSimulatedResponse(decision.intent, interaction.input)
